...
/* === v38.3 question types upgrade === */
(function(){
  function supportsAdvancedType(q){
    const t = String(q?.type || '').toLowerCase();
    return ['truefalse','true_false','input','typemissingletter','reorder','reorderletters','match','matchimagetoword','listen','listenandchoose','drag','dragdrop'].includes(t);
  }
  function normalizedType(q){
    const t = String(q?.type || '').toLowerCase();
    if (['truefalse','true_false'].includes(t)) return 'truefalse';
    if (['input','typemissingletter'].includes(t)) return 'input';
    if (['reorder','reorderletters'].includes(t)) return 'reorder';
    if (['match','matchimagetoword'].includes(t)) return 'match';
    if (['listen','listenandchoose'].includes(t)) return 'listen';
    if (['drag','dragdrop'].includes(t)) return 'drag';
    return 'choice';
  }
  ...
  if (_origLoadQuestion) {
    window.loadQuestion = function(){
      ...
      try{
        ...
        if(!q) return;
        renderAdvancedQuestionUI(q);
      }catch(e){}
    };
  }

  window.renderAdvancedQuestionUI = function(q){
    const type = normalizedType(q);
    const optionsWrap = document.getElementById('optionsWrap') || document.getElementById('optionsContainer');
    const imageWrap = document.getElementById('questionImageWrap') || document.getElementById('imageWrapper');
    const badge = document.getElementById('questionTypeBadge') || document.getElementById('questionType');
    if(!optionsWrap) return;
    if (badge) {
      const map = {
        truefalse: 'True / False',
        input: 'Type Missing Letter',
        reorder: 'Reorder Letters',
        match: 'Match Image To Word',
        listen: 'Listen & Choose',
        drag: 'Drag & Drop'
      };
      if (type !== 'choice') badge.textContent = map[type] || badge.textContent;
    }
    ...
    if (type === 'truefalse') {
      ...
      ['True','False'].forEach(opt => {
        ...
        btn.textContent = opt;
        btn.addEventListener('click', ()=> selectAdvancedAnswer(opt, btn));
    ...
    if (type === 'input') {
      optionsWrap.innerHTML = `
        <div class="advanced-card">
          <input id="advancedTextAnswer" class="advanced-input" placeholder="Type the missing letter or word">
          <button class="main-btn" id="advancedSubmitBtn">Submit Answer</button>
        </div>
      `;
      const btn = document.getElementById('advancedSubmitBtn');
      btn?.addEventListener('click', ()=>{
        const val = (document.getElementById('advancedTextAnswer')?.value || '').trim();
        selectAdvancedAnswer(val, null);
      });
      return;
    }

    if (type === 'reorder') {
      const letters = Array.isArray(q.letters) && q.letters.length ? q.letters.slice() : String(q.answer || '').split('').sort(()=>Math.random()-0.5);
      optionsWrap.innerHTML = `
        <div class="advanced-card">
          <div class="letters-bank" id="lettersBank"></div>
          <div class="drop-zone" id="reorderDropZone"></div>
          <div class="actions-row"><button class="ghost-btn" id="reorderClearBtn">Clear</button><button class="main-btn" id="reorderSubmitBtn">Submit Answer</button></div>
        </div>
      `;
      const bank = document.getElementById('lettersBank');
      const zone = document.getElementById('reorderDropZone');
      const renderLetters = ()=>{
        bank.innerHTML = '';
      ...
      renderLetters();
      document.getElementById('reorderClearBtn')?.addEventListener('click', ()=> zone.innerHTML = '');
      document.getElementById('reorderSubmitBtn')?.addEventListener('click', ()=>{
        const val = Array.from(zone.querySelectorAll('.placed-letter')).map(n=>n.textContent).join('');
        selectAdvancedAnswer(val, null);
      });
      return;
    }

    if (type === 'match' || type === 'listen' || type === 'drag') {
      // keep multiple-choice UI, but enhance labels/behavior
      if (type === 'listen') {
          ...
          readBtn.onclick = ()=>{
            try{
              if('speechSynthesis' in window){
                ...
                const u = new SpeechSynthesisUtterance(String(q.speech || q.answer || q.text || ''));
                u.lang = 'en-US';
      ...
      if (type === 'drag') {
        optionsWrap.classList.add('drag-mode');
        Array.from(optionsWrap.querySelectorAll('.option-btn')).forEach(btn=>{
          btn.setAttribute('draggable','true');
          btn.addEventListener('dragstart', ev => ev.dataTransfer.setData('text/plain', btn.textContent));
        });
        const drop = document.createElement('div');
        drop.className = 'drop-zone drag-drop-zone';
        ...
        optionsWrap.parentNode.insertBefore(drop, optionsWrap.nextSibling);
      }
      Array.from(optionsWrap.querySelectorAll('.option-btn')).forEach(btn=>{
        btn.onclick = ()=> selectAdvancedAnswer(btn.textContent, btn);
      });
      return;
    }

    // choice fallback: keep existing
  };

  window.selectAdvancedAnswer = function(rawValue, btnEl){
    try{
      const q = quizState?.questions?.[quizState.index];
      if(!q) return;
      const value = String(rawValue || '').trim();
      const answer = String(q.answer || '').trim();
      const ok = value.toLowerCase() === answer.toLowerCase();
      if (typeof handleAdvancedAnswerOutcome === 'function') {
        handleAdvancedAnswerOutcome(ok, btnEl, value);
        return;
      }
      // fallback to existing engine
      if (typeof handleAnswer === 'function') {
        handleAnswer(btnEl || { textContent: value, classList:{ add(){} } }, ok);
      }
    }catch(e){}
  };

  window.handleAdvancedAnswerOutcome = function(isCorrect, btnEl, value){
    try{
      if (window.quizState && quizState.locked) return;
      if (window.quizState) quizState.locked = true;

      const q = quizState?.questions?.[quizState.index];
      const optionsWrap = document.getElementById('optionsWrap') || document.getElementById('optionsContainer');
      const allBtns = optionsWrap ? optionsWrap.querySelectorAll('.option-btn') : [];
      allBtns.forEach(btn => btn.classList.add('disabled'));

      if (btnEl && btnEl.classList) btnEl.classList.add(isCorrect ? 'correct' : 'wrong');
      ...
      const timerEnabled = typeof isTimerEnabledForGrade === 'function' ? isTimerEnabledForGrade() : true;
      const timeLeft = Number(document.getElementById('timer')?.textContent || 0);
      const points = isCorrect ? (timerEnabled ? 10 + Math.max(0, timeLeft) : 10) : 0;

      if (window.quizState) {
        quizState.score = (quizState.score || 0) + points;
        if (quizState.records) {
          quizState.records.push({
            question: q?.text || '',
            correct: !!isCorrect,
            answerGiven: value,
      ...
      if (scoreEl && window.quizState) scoreEl.textContent = quizState.score || 0;

      setTimeout(()=>{
        if (typeof goToNextQuestion === 'function') goToNextQuestion();
        else if (typeof nextQuestion === 'function') nextQuestion();
      }, 800);
    }catch(e){}
  };

  // safer add-question validation for new types
  const _origAddCustomQuestion = window.addCustomQuestion;
  if (_origAddCustomQuestion) {
    window.addCustomQuestion = function(){
      const type = (document.getElementById('newQType')?.value || '').trim().toLowerCase();
      const answer = (document.getElementById('newQAnswer')?.value || '').trim();
      const options = (document.getElementById('newQOptions')?.value || '').split('|').map(s=>s.trim()).filter(Boolean);
      if (['truefalse','true_false'].includes(type) && !['true','false'].includes(answer.toLowerCase())) {
        alert(getLang()==='ar' ? 'إجابة True/False يجب أن تكون True أو False.' : 'True/False answer must be True or False.');
        return;
      }
      if (['choice','match','listen','drag','picture'].includes(type) && options.length && !options.some(o => o.toLowerCase() === answer.toLowerCase())) {
        alert(getLang()==='ar' ? 'الإجابة الصحيحة يجب أن تكون ضمن الاختيارات.' : 'Correct answer must exist in the options.');
        return;
      }
      _origAddCustomQuestion();
    };
  }

  // extend preview
  const _origQuestionEditorCard = window.questionEditorCard;
  if (_origQuestionEditorCard) {
    window.questionEditorCard = function(question){
      let html = _origQuestionEditorCard(question);
      if (html && !html.includes('preview-question-btn')) {
        html = html.replace('</div></div>$', `<button class="ghost-btn preview-question-btn">Preview</button></div></div>`);
      }
      return html;
    };
  }

  const _origBindQuestionEditorActions = window.bindQuestionEditorActions;
  if (_origBindQuestionEditorActions) {
    window.bindQuestionEditorActions = function(){
      _origBindQuestionEditorActions();
      document.querySelectorAll('.preview-question-btn').forEach(btn=>{
        if(btn.dataset.bound==='1') return;
        btn.dataset.bound='1';
        btn.addEventListener('click', ()=>{
          const card = btn.closest('.question-edit-card');
          if(!card) return;
          const text = card.querySelector('.qe-text')?.value || '';
          const skill = card.querySelector('.qe-skill')?.value || '';
          const type = card.querySelector('.qe-type')?.value || '';
          const answer = card.querySelector('.qe-answer')?.value || '';
          const image = card.querySelector('.qe-image')?.value || '';
          const options = (card.querySelector('.qe-options')?.value || '').split('|').map(s=>s.trim()).filter(Boolean);
