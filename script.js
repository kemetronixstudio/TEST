...
(function(){
  ...
  async function stablePdfBlob(){
    const area = document.getElementById('certificateArea');
    if (!area || !window.html2canvas || !window.jspdf) throw new Error('PDF libs missing');
    const imgs = Array.from(area.querySelectorAll('img'));
    await Promise.all(imgs.map(img => img.complete ? Promise.resolve() : new Promise(res => { img.onload = img.onerror = ()=>res(); })));
    await new Promise(res => setTimeout(res, 180));
    const canvas = await html2canvas(area, {scale:2, backgroundColor:'#fffdf4', useCORS:true, logging:false});
  ...
  window.makeCertificatePdfBlob = stablePdfBlob;

  window.renderCertificate = function(){
    try{
      if (!document.getElementById('certificateArea')) return;
      const data = (typeof readJson === 'function' && typeof storeKeys !== 'undefined')
        ? readJson(storeKeys.cert, null)
        : JSON.parse(localStorage.getItem('kgEnglishCertificate') || 'null');
      if (!data) return;
      const lang = typeof getLang === 'function' ? getLang() : (data.lang || 'en');
      document.body.dataset.lang = lang;
      const setText = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val ?? ''; };
      const title = lang === 'ar' ? 'شهادة تقدير' : ((translations[lang] && translations[lang].certificateTitle) || 'Certificate of Achievement');
      setText('certTitle', title);
      setText('certTopText', title);
      setText('certSubtitle', (translations[lang] && translations[lang].presentedTo) || 'Proudly presented to');
      setText('certStudentName', data.studentName || 'Student');
      setText('certMessage', (translations[lang] && translations[lang].completedMsg) || 'has completed the quiz successfully.');
      setText('labelGrade', (translations[lang] && translations[lang].grade) || 'Grade');
      setText('labelLevel', (translations[lang] && translations[lang].level) || 'Quiz Level');
      setText('labelScore', (translations[lang] && translations[lang].score) || 'Score');
      setText('labelDate', (translations[lang] && translations[lang].date) || 'Date');
      setText('labelStrengths', (translations[lang] && translations[lang].strengths) || 'Strengths');
      setText('labelWeak', (translations[lang] && translations[lang].focus) || 'Focus More On');
      setText('labelAdvice', (translations[lang] && translations[lang].advice) || 'Teacher Advice');
      setText('labelTeacher', (translations[lang] && translations[lang].teacher) || 'Teacher');
      setText('labelResult', (translations[lang] && translations[lang].result) || 'Result');
      setText('certGrade', data.grade || '');
      setText('certLevel', `${data.quizLevel || ''} / ${data.questionCount || ''}`);
      setText('certScore', `${data.score ?? ''} (${data.percent ?? 0}%)`);
      setText('certDate', data.date || '');
      setText('certStrengths', Array.isArray(data.strengths) ? data.strengths.join(', ') : (data.strengths || ''));
      setText('certWeaknesses', Array.isArray(data.weaknesses) ? data.weaknesses.join(', ') : (data.weaknesses || ''));
      setText('certAdvice', data.advice || '');
      setText('certRemark', data.remark || '');
      const qr = document.getElementById('certQr');
      if (qr){
        qr.innerHTML = '';
        const homeUrl = location.href.replace('certificate.html','index.html');
        if (window.QRCode){
          new QRCode(qr, { text: homeUrl, width:110, height:110 });
        } else {
          qr.textContent = homeUrl;
        }
      }
      const dl = document.getElementById('downloadPdfBtn');
      if (dl && !dl.dataset.v385){
        dl.dataset.v385 = '1';
        dl.onclick = async ()=>{
          try{
            const blob = await stablePdfBlob();
            const url = URL.createObjectURL(blob);
      ...
      const sh = document.getElementById('shareCertBtn');
      if (sh && !sh.dataset.v385){
        sh.dataset.v385 = '1';
        sh.onclick = async ()=>{
          try{
            const blob = await stablePdfBlob();
  ...
  function robustAdminLoginBind(){
    if (document.body.dataset.page !== 'admin') return;
    const oldBtn = document.getElementById('adminLoginBtn');
    if (!oldBtn) return;
    ...
    newBtn.addEventListener('click', ()=>{
      try{
        ...
        if (saveAccBtn && !saveAccBtn.dataset.v385){
          saveAccBtn.dataset.v385 = '1';
          saveAccBtn.onclick = (e)=>{ e.preventDefault(); if (typeof saveAccessAccountFromAdmin === 'function') saveAccessAccountFromAdmin(); };
        }
      }catch(err){
        alert('Admin panel failed to open.');
      }
    });
  }
  ...
  window.addEventListener('load', ()=>{
    setTimeout(()=>{
      robustAdminLoginBind();
      if (document.body.dataset.page === 'certificate') window.renderCertificate();
    }, 50);
  });
})();
