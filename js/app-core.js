...
function getTimerSettings(){ return readJson(storeKeys.timerSettings, {kg1:true, kg2:true, grade1:true, grade2:true, grade3:true, grade4:true, grade5:true, grade6:true}); }
...
function renderStudentHistory(name, grade){ const box = $('#studentHistory'); if (!box) return; const lang = getLang(); const progress = getProgress(); const item = progress[String(name || '').trim().toLowerCase()]; if (!item){ box.innerHTML = ''; return; } const prev = item.scores || []; const improve = prev.length >= 2 ? `${translations[lang].improvedFrom} ${prev[0]}% ${translations[lang].to} ${prev[prev.length-1]}%` : `${translations[lang].lastScore}: ${item.lastScore}%`; const weak = (item.weakTimeline?.[item.weakTimeline.length-1]?.weaknesses || []).slice(0,2).join(', '); const parts = [`<span class="history-chip em">${escapeHtml(improve)}</span>`, `<span class="history-chip">${escapeHtml(`${translations[lang].bestScore}: ${item.bestScore}%`)}</span>`]; if (weak) parts.push(`<span class="history-chip">${escapeHtml(`${translations[lang].historyWeak}: ${weak}`)}</span>`); box.innerHTML = parts.join(''); }
...
function initQuiz(){
  ...
  const getStudentCloudClient = ()=>window.studentCloud || null;
  ...
  { const studentCloud = getStudentCloudClient(); if (studentCloud && typeof studentCloud.ensureQuizIdentityFields === 'function') studentCloud.ensureQuizIdentityFields(grade.toUpperCase()); }
  studentNameInput?.addEventListener('input', updateHistory); updateHistory();
  function collectStudentIdentity(){
    const studentCloud = getStudentCloudClient(); if (studentCloud && typeof studentCloud.collectIdentity === 'function') return studentCloud.collectIdentity(grade.toUpperCase());
  ...
  async function promptForQuizPassword(){
    ...
    const entered = await window.askTextInput(`This quiz is protected. Enter password. If you do not know it, contact Dr. Tarek 01032188008.`) || '';
    if (String(entered) !== String(access.password)){ alert('Wrong password. Contact Dr. Tarek 01032188008.'); return false; }
    return true;
  }
...
async function exportDashboardJpeg(){ const node = $('#adminDashboardContent'); if (!node) return; if (typeof html2canvas === 'undefined') { alert('Image export tools are not available right now. Please reload the page and try again.'); return; } const canvas = await html2canvas(node, {scale:2, backgroundColor:'#fffdf4', useCORS:true}); canvas.toBlob(blob => { if (!blob) return; const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'teacher-dashboard.jpeg'; a.click(); setTimeout(()=>URL.revokeObjectURL(url),1500); }, 'image/jpeg', 0.95); }
...
(function(){
  ...
  window.__legacyGetLoginAccount = function(user, pass){
...
(function(){
  ...
  function allPerms(){ return Array.isArray(PERMISSIONS) ? [...PERMISSIONS] : ['dashboard','levelVisibility','timerSettings','quizAccess','teacherTest','bulkQuestions','questionBank','classManager','homeworkAnalytics','homeworkBuilder','homeworkReports','accountManager']; }
...
(function(){
  ...
  function builtInLookup(user, pass){
    ...
    const perms = Array.isArray(window.PERMISSIONS) ? window.PERMISSIONS.slice() : ['dashboard','levelVisibility','timerSettings','quizAccess','teacherTest','bulkQuestions','questionBank','classManager','homeworkAnalytics','homeworkBuilder','homeworkReports','accountManager'];
    return {
...
(function(){
  ...
  function mergeIntoPools(){
    ...
    if (window.__kgBulkPackageMergedOnce__) return;
    window.__kgBulkPackageMergedOnce__ = true;
  ...
  function appendCountNote(){
    document.querySelectorAll('.question-count-note').forEach(function(n){ n.remove(); });
...
(function(){
  ...
  function patchRenderStoredQuestions(){
    ...
    window.renderStoredQuestions = function(){
      ...
      list.innerHTML = items.length ? items.map(window.questionEditorCard || function(q){ return '<div class="stored-question"><h4>' + escapeHtml(q.text || 'Question') + '</h4></div>'; }).join('') : '<div class="stored-question"><h4>No questions yet.</h4><p>Add questions from the editor above.</p></div>';
...
/* ---- BEGIN final-stability-patch.js ---- */
(function(){
  ...
  function stableGetLoginAccount(user, pass){
    return null;
  }
  window.getLoginAccount = stableGetLoginAccount;
  try { window.ADMINS = Array.isArray(ADMINS) ? ADMINS.slice() : []; } catch (error) {}

  async function stableAdminLogin(event){
    if (event) {
      event.preventDefault();
      if (typeof event.stopImmediatePropagation === 'function') event.stopImmediatePropagation();
    }
    if (window.__kgAdminLoginBusy) return;
    window.__kgAdminLoginBusy = true;
    try {
      ...
      var account = stableGetLoginAccount(user, pass);
      ...
      if (typeof wireCollapseButtons === 'function') wireCollapseButtons();
      if (typeof wireQuestionFilterButtons === 'function') wireQuestionFilterButtons();
      var classManager = document.querySelector('[data-section-key="classManager"]');
      if (classManager) { classManager.classList.remove('hidden'); classManager.style.display = ''; }
      var hwAnalytics = document.getElementById('homeworkAnalyticsSection');
      if (hwAnalytics) hwAnalytics.style.display = '';
    } finally {
      window.__kgAdminLoginBusy = false;
    }
  }
  ...
  function safeRenderStoredQuestions(){
    ...
    if (!list || typeof window.collectQuestionsWithMeta !== 'function') return;
    var items = [];
    try {
      items = [].concat(window.collectQuestionsWithMeta('kg1') || [], window.collectQuestionsWithMeta('kg2') || []).map(function(q){
        return typeof window.applyQuestionOverrides === 'function' ? window.applyQuestionOverrides(q) : q;
      }).filter(function(q){ return !(q && q._deleted); });
    } catch (error) { return; }
    if (!items.length) {
      list.innerHTML = '<div class="stored-question"><h4>No questions yet.</h4><p>Add questions from the editor above.</p></div>';
    } else {
      var renderer = window.questionEditorCard;
      list.innerHTML = items.map(function(q){
        if (typeof renderer === 'function') return renderer(q);
        return '<div class="stored-question"><h4>' + escapeSafe(q && q.text || 'Question') + '</h4></div>';
      }).join('');
    }
    if (typeof window.bindQuestionEditorActions === 'function') window.bindQuestionEditorActions();
  }
  window.renderStoredQuestions = safeRenderStoredQuestions;
...
/* v17 translation and play consistency patch */
(function(){
  try {
    if (typeof window === 'undefined' || !window.translations) return;
    var t = window.translations;
    if (t.en) {
      t.en.playChooseStage = 'Difficulty follows the selected grade';
      t.en.playTimeOverResult = 'Time is over. You scored {score} points.';
      t.en.playGreatResult = 'Amazing. Final score: {score}';
      t.en.homeworkCardDesc = 'Open homework tasks by class, answer without instant right or wrong feedback, and submit to Dr. Tarek.';
      t.en.homeworkCardBtn = 'Open Homework';
    }
    if (t.ar) {
      t.ar.playChooseStage = 'يتم تحديد الصعوبة تلقائيًا حسب الصف';
      t.ar.playTimeOverResult = 'انتهى الوقت. لقد حصلت على {score} نقطة.';
      t.ar.playGreatResult = 'رائع! النتيجة النهائية: {score}';
      t.ar.homeworkCardDesc = 'افتح واجبات الصف، وأجب بدون إظهار الصحيح أو الخطأ فورًا، ثم أرسل الحل إلى د. طارق.';
      t.ar.homeworkCardBtn = 'افتح الواجب';
    }
  } catch (e) {}
})();
  ...
  try {
    if (typeof window !== 'undefined' && window.translations) {
      if (window.translations.en) {
        window.translations.en.playChooseStage = 'Difficulty follows the selected grade';
        window.translations.en.homeworkCardDesc = 'Open homework tasks by class, answer without instant right or wrong feedback, and submit to Dr. Tarek.';
        window.translations.en.homeworkCardBtn = 'Open Homework';
      }
      if (window.translations.ar) {
        window.translations.ar.playChooseStage = 'يتم تحديد الصعوبة تلقائيًا حسب الصف';
        window.translations.ar.homeworkCardDesc = 'افتح واجبات الصف، وأجب بدون إظهار الصحيح أو الخطأ فورًا، ثم أرسل الحل إلى د. طارق.';
        window.translations.ar.homeworkCardBtn = 'افتح الواجب';
      }
    }
  } catch (e) {}
})();
