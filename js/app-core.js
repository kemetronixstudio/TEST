...
const ADMINS = [{ user: 'admin', pass: 'Admin@123' }];
...
function getLoginAccount(user, pass){
  ...
  const hard = ADMINS.find(a => String(a.user || '').trim().toLowerCase() === lookup && String(a.pass || '') === password);
...
(function(){
  ...
  function stableGetLoginAccount(user, pass){
    ...
    var hard = (Array.isArray(window.ADMINS) ? window.ADMINS : (typeof ADMINS !== 'undefined' ? ADMINS : [])).find(function(a){
      return normalizeUserValue(a && a.user) === lookup && String((a && a.pass) || '') === password;
    });
    if (hard) return { user: hard.user, role: 'admin', permissions: (typeof defaultAdminPermissions === 'function' ? defaultAdminPermissions() : []), builtIn: true };
    var accounts = [];
