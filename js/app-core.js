...
function getLoginAccount(user, pass){
  const lookup = String(user || '').trim().toLowerCase();
  const password = String(pass || '');
  const match = getAccessAccounts().find(a => String(a.user || '').trim().toLowerCase() === lookup && String(a.pass || '') === password);
...
(function(){
  ...
  function stableGetLoginAccount(user, pass){
    var lookup = normalizeUserValue(user);
    var password = String(pass || '');
    var accounts = [];
    try { accounts = (typeof getAccessAccounts === 'function' ? getAccessAccounts() : []) || []; } catch (error) {}
    var match = accounts.find(function(a){ return normalizeUserValue(a && a.user) === lookup && String((a && a.pass) || '') === password; });
