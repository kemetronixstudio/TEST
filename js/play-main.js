...
function getSelectedGameMode(){
  ...
  if (active) return String(active.getAttribute('data-mode-value') || active.getAttribute('data-mode-card') || 'question_timer');
...
(function(){
  ...
  async function localPlayRequest(path, options){
    ...
    if (action === 'submit') {
      ...
      const existing = store.scores[key] || { studentName: identity.name || '', name: identity.name || '', studentId: identity.studentId || '', grade: identity.grade || '', bestScore: 0, attempts: 0, lastPlayed:'', updatedAt:'', lastScore:0, stage: result.stageLabel || result.stage || '' };
      const score = Number(result.score || 0) || 0;
      existing.studentName = identity.name || existing.studentName || existing.name;
      existing.name = identity.name || existing.name || existing.studentName;
      ...
      existing.updatedAt = existing.lastPlayed;
  ...
  function buildIdentity(){ const name = String($('playStudentName')?.value || '').trim(); const studentId = String($('playStudentId')?.value || '').trim(); const rawGrade = String($('playStudentGrade')?.value || 'KG1').trim(); const grade = (typeof normalizeGradeKey === 'function' ? normalizeGradeKey(rawGrade) : rawGrade); if (!name) throw new Error(tr('playEnterName')); return { name, studentId, grade, isGuest:true, className:'Play & Test' }; }
  function formatTime(seconds){ const total = Math.max(0, Number(seconds||0)); const mins = Math.floor(total / 60); const secs = total % 60; return mins > 0 ? String(mins).padStart(2,'0') + ':' + String(secs).padStart(2,'0') : String(secs); }
  ...
  function updateBadges(){ if (!state) return; $('playPlayerBadge').textContent = tr('playPlayer') + ': ' + state.identity.name; $('playStageBadge').textContent = tr('playStage') + ': ' + getStageLabel(state.stage); $('playQuestionBadge').textContent = tr('playQuestion') + ' ' + (state.currentIndex + 1); $('playScoreBadge').textContent = tr('playScore') + ': ' + state.score; const timerValue = state.mode === 'total_timer' ? state.totalTimeLeft : state.timeLeft; $('playTimerBadge').textContent = tr('playTime') + ': ' + (state.mode === 'endless' ? 'Off' : formatTime(timerValue)); $('playTimerBadge').classList.toggle('timer-warning', state.mode !== 'endless' && Number(timerValue) <= 5); updateProgress(); }
  ...
  function renderLeaderboard(data){
    ...
    const displayName = (row) => row.studentName || row.name || '-';
    const playedAt = (row) => row.updatedAt || row.lastPlayed || '';
    $('playTop3List').innerHTML = top3.length ? top3.map((row, index) => `<div class="play-top3-item rank-${index+1}"><div class="play-medal">${index===0?'🥇':index===1?'🥈':'🥉'}</div><div><strong>${escapeHtml(displayName(row))}</strong><span>${escapeHtml(row.grade || '-')}</span><small>${escapeHtml(String(row.bestScore || 0))} pts</small></div><div class="play-medal-score">${escapeHtml(String(row.bestScore || 0))}</div></div>`).join('') : `<div class="play-top3-empty">${escapeHtml(tr('playNoTop3'))}</div>`;
    $('playLeaderboardBody').innerHTML = leaders.length ? leaders.map((row, index) => `<tr><td>${index+1}</td><td>${escapeHtml(displayName(row))}</td><td>${escapeHtml(row.grade || '-')}</td><td>${escapeHtml(row.studentId || '-')}</td><td>${escapeHtml(String(row.bestScore || 0))}</td><td>${escapeHtml(String(row.attempts || 0))}</td><td>${escapeHtml(playedAt(row) ? new Date(playedAt(row)).toLocaleString() : '-')}</td></tr>`).join('') : `<tr><td colspan="7">${escapeHtml(tr('playNoLeaderboard'))}</td></tr>`;
  }
  ...
  function startTimer(){
    ...
    if (state.mode === 'total_timer') {
      if (!Number.isFinite(Number(state.totalTimeLeft)) || Number(state.totalTimeLeft) <= 0) state.totalTimeLeft = state.totalSeconds || selectedTotalSeconds();
      updateBadges();
  ...
  function renderQuestion(){ if (!state) return; questionRenderToken += 1; answerLock = false; if (autoNextTimer) { clearTimeout(autoNextTimer); autoNextTimer = null; } if (state.currentIndex >= state.questions.length) { finishQuiz(false, false).catch(()=>{}); return; } showSection('playQuizCard'); const q = state.questions[state.currentIndex]; const answeredCurrent = state.answers.find(a => a.index === state.currentIndex); const resumingSameQuestion = state._resumeIndex === state.currentIndex && !answeredCurrent; if (state.mode === 'question_timer' && (!resumingSameQuestion || !Number.isFinite(Number(state.timeLeft)) || Number(state.timeLeft) <= 0 || answeredCurrent)) state.timeLeft = QUESTION_SECONDS; state._resumeIndex = null; updateBadges(); $('playQuestionText').textContent = q.text;
  ...
  $('playOptions').innerHTML = q.options.map((opt, idx) => `<button type="button" class="play-option-btn" data-option-index="${idx}">${escapeHtml(opt)}</button>`).join(''); $('playNextBtn').disabled = true; startTimer(); }
