...
async function readLimiterRecord(bucketKey) {
  try {
    if (getKvConfig()) return await kvGetJson('kgRateLimit:' + bucketKey, null);
  } catch (error) {}
  const fileStore = readLimiterFileStore();
  if (fileStore && fileStore[bucketKey]) return fileStore[bucketKey];
  return limiterStore().get(bucketKey) || null;
}
async function writeLimiterRecord(bucketKey, record) {
  try {
    if (getKvConfig()) {
  ...
  limiterStore().set(bucketKey, record);
}
async function checkRateLimit(req, key) {
  ...
  const rec = await readLimiterRecord(bucketKey);
  if (!rec || rec.resetAt <= now) {
    const next = { count: 1, resetAt: now + windowMs };
    await writeLimiterRecord(bucketKey, next);
    return { ok: true, remaining: max - 1 };
  }
  if (rec.count >= max) return { ok: false, retryAfter: Math.max(1, Math.ceil((rec.resetAt - now)/1000)) };
  rec.count += 1;
  await writeLimiterRecord(bucketKey, rec);
