...
const DEFAULT_RUNTIME_DATA_DIR = process.env.RUNTIME_DATA_DIR || (process.env.VERCEL ? path.join('/tmp', 'kg-quiz-runtime') : path.join(os.tmpdir(), 'kg-quiz-runtime'));
function deriveStableFallbackSecret(){
  try {
    const builtinPath = process.env.BUILTIN_ADMINS_FILE || path.join(process.cwd(), 'server-config', 'builtin-access-accounts.json');
    const builtinRaw = fs.existsSync(builtinPath) ? String(fs.readFileSync(builtinPath, 'utf8') || '') : JSON.stringify(EMBEDDED_BUILTIN_ADMINS);
    const seed = 'kg-access-fallback|' + builtinRaw + '|' + String(process.env.ACCESS_ACCOUNTS_STORAGE_KEY || 'kg:access_accounts:v1');
    return crypto.createHash('sha256').update(seed).digest('hex');
  } catch (error) {
    return crypto.createHash('sha256').update('kg-access-fallback|default').digest('hex');
  }
}
function readOrCreateLocalSecret(){
  ...
  if (process.env.VERCEL) return deriveStableFallbackSecret();
  const secretPath = process.env.ACCESS_ACCOUNTS_SESSION_SECRET_FILE || path.join(DEFAULT_RUNTIME_DATA_DIR, 'access-session-secret.txt');
  try {
    ...
    return deriveStableFallbackSecret();
  }
}
...
const RUNTIME_DATA_DIR = DEFAULT_RUNTIME_DATA_DIR;
...
const BUILTIN_FILE_PATH = process.env.BUILTIN_ADMINS_FILE || path.join(process.cwd(), 'server-config', 'builtin-access-accounts.json');
...
if (process.env.VERCEL && !String(process.env.ACCESS_ACCOUNTS_SESSION_SECRET || process.env.SESSION_SECRET || '').trim()) { console.warn('[kg-access] ACCESS_ACCOUNTS_SESSION_SECRET is not set. Using a deterministic fallback secret for this build; set an explicit secret in Vercel for production session continuity.'); }
...
function parseBuiltinAdmins() {
  ...
  const fromFile = normalizeBuiltinAdmins(readBuiltinAdminsFile());
  return fromFile.length ? fromFile : normalizeBuiltinAdmins(EMBEDDED_BUILTIN_ADMINS);
}
...
function verifyPassword(password, storedValue) {
  ...
  if (!value.startsWith('pbkdf2$')) {
    if (!ALLOW_PLAIN_PASSWORDS) return false;
    try {
...
function loginGuardKey(user, ip) {
  var normUser = normalizeUser(user) || 'unknown-user';
  var normIp = String(ip || 'unknown').trim().toLowerCase() || 'unknown-ip';
...
async function authenticate(user, password, req) {
  ...
  const ok = !!match && verifyPassword(password, match.passwordHash);
  if (!ok) {
...
async function authenticateFromHeaders(req) {
  ...
  const limited = await checkRateLimit(req, 'header-auth:' + user.toLowerCase());
  if (!limited.ok) {
