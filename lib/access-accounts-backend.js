...
function deriveStableFallbackSecret(){
  try {
    const builtinPath = process.env.BUILTIN_ADMINS_FILE || path.join(process.cwd(), 'data', 'builtin-access-accounts.json');
    const builtinRaw = fs.existsSync(builtinPath) ? String(fs.readFileSync(builtinPath, 'utf8') || '') : '';
